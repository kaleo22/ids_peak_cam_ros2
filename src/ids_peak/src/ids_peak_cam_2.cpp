...
std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("ids_peak_cam_2");
  ...
  //img_pub_(it_->advertiseCamera("ids_cam/image_raw",1))
{
  rclcpp::Node* rawPtr = nh_.get();
   ...
   // ,"4104657655" .param<std::string> param<int>
...
void ids_cam::camOpen()
{
  ...
  {
      auto logger = rclcpp::get_logger("Err_Log");
  ...
  if(dev_ == nullptr)
  {
    auto logger = rclcpp::get_logger("Ser_log");
  ...
  {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("snn_Log"), "["<<serNo_<<"] name not valid"
...
void ids_cam::camConfig()
{
  ...
  auto logger1 = rclcpp::get_logger("expoT_Log");
...
void ids_cam::camAcquisitionStop()
{
  ...
  auto logger = rclcpp::get_logger("Warn2_log");
...
{
  sensor_msgs::msg::CameraInfo::Ptr ci(new sensor_msgs::msg::CameraInfo(cinfo_->getCameraInfo()));
