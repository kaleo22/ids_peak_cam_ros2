...
std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("ids_peak_cam_3");
...
void ids_cam::camOpen()
{
  ...
  {
      auto logger = rclcpp::get_logger("CamNotFound");
  ...
  if(dev_ == nullptr)
  {
    auto logger = rclcpp::get_logger("ErrLog");
  ...
  {
    auto logger = rclcpp::get_logger("NameLog");
...
void ids_cam::camConfig()
{
  ...
  auto logger1 = rclcpp::get_logger("ExpTiLog");
  auto logger2 = rclcpp::get_logger("FraRaLog"); 
