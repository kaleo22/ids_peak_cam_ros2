...
ids_cam::ids_cam(rclcpp::Node::make_shared("ids_peak_cam_test")):
  nh_(nh)
...
void ids_cam::camOpen()
{
  ...
  {
      RCLCPP_ERROR("No camera found. Exiting program.");
      peak::Library::Close();
      //std::exit(0);
      rclcpp::shutdown();
      //return 0;
  ...
  for (const auto& descriptor: deviceManager.Devices())
  {
    ...
    {	
        ...
        size_t i = 0;
        RCLCPP_INFO_ONCE("Devices available: ");
        for (const auto& deviceDescriptor : deviceManager.Devices())
        {
            RCLCPP_INFO("%lu: %s", i, deviceDescriptor->DisplayName().c_str());
            ++i;
        }
  ...
  if(dev_ == nullptr)
  {
	  RCLCPP_ERROR("SerNO Wrong. Exiting node.");
     peak::Library::Close();
     rclcpp::shutdown();
  }
  ...
  {
    RCLCPP_WARN_STREAM(rclcpp::get_logger, "["<<serNo_<<"] name not valid"
...
void ids_cam::camConfig()
{
  ...
  RCLCPP_INFO("CURRENT EXPOSURE TIME: %f", exposureTime_current);
  RCLCPP_INFO("CURRENT EXPOSURE TIME: %f", frameRate_current);
}
...
void ids_cam::camAcquisitionStop()
{
  ...
  RCLCPP_WARN("WARNING: Ignoring that TL failed to stop acquisition on datastream.");
  }
